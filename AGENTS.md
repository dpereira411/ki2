...
- In that same shared `parseSchText()` path, keep the branch-head token and fallback `Expecting(...)` text aligned with the full routine body too: `exclude_from_sim, at, shape, length, fields_autoplaced, effects, iref, uuid or property`. Do not leave stale copied head strings that omit valid branches owned by the routine.
...
- In `parseSchTextBoxContent()` and table-cell textbox parsing, keep that local child list ordered to the actual routine body too: `... stroke, fill, effects, margins ...`. Do not leave stale `margins, effects` ordering once the parser body handles `effects` before `margins`.
- In table-cell textbox parsing, keep `span` in the local branch-head/fallback text where the routine actually handles it too: `... at, size, span, stroke, fill, effects, margins ...`. Do not leave `span` tacked on at the end once the parser body matches it earlier.
...
- In `parseBusEntry()`, keep that local child list ordered to the routine body too: `at, size, stroke or uuid`. Do not leave the stale `uuid or stroke` ordering once `stroke` is matched before `uuid` in the parser body.
...
- In `parseSheet()`, keep the local branch-head token and fallback `Expecting(...)` text aligned with the actual child set too: the routine owns `fill`, not `background`. Do not leave stale copied `background` text once the parser body matches on `fill`.
...
- In `parseSheet()`, keep the local branch-head token and fallback `Expecting(...)` text aligned with the full routine body. This parser branch also owns `exclude_from_sim`, `in_bom`, `on_board`, `dnp`, and `fields_autoplaced`, so do not leave those out of the local head/fallback text.
...
- In `parseSchematicSymbol()`, keep the local branch-head token and fallback `Expecting(...)` text aligned with the full routine body too. This parser branch also owns `convert`, `body_style`, `unit`, `in_pos_files`, and `fields_autoplaced`, so do not leave those out of the local head/fallback text.
...
- In library `parseSymbolTextBox()` / `parse_lib_text_box_draw_item()`, keep the branch-head token and fallback `Expecting(...)` text aligned with the full routine body too: `start, end, at, size, stroke, fill, margins, or effects`. Do not leave `start/end/margins` out of the local fallback text.
