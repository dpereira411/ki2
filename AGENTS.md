...
- In `parseSheet()`, keep KiCad's literal fallback `Expecting(...)` text even where it is stale. Upstream still says `at, size, stroke, background, instances, uuid, property, or pin` there.
...
- In `parseSchematicSymbol()`, keep KiCad's literal fallback `Expecting(...)` text even where it omits valid branches. Upstream still says `lib_id, lib_name, at, mirror, uuid, exclude_from_sim, on_board, in_bom, dnp, default_instance, property, pin, or instances` there.
