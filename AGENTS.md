...
- In that same library `parseProperty()` duplicate-user-field path, do not cap suffix retries at a repo-local `_9` ceiling. Keep suffixing until a free field name is found, like upstream duplicate-field renaming flow, rather than silently dropping later duplicates.
...
- In those same nested symbol/sheet `variant` flows, do not insert the variant into the owning instance map on every child branch. Finish mutating the local variant first, then store it once after the nested variant loop, or late `name` tokens will leave a bogus provisional empty-name variant behind.
