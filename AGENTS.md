...
- Keep the `embedded_files` section head and final close-token ownership on the shared embedded-files parser too. Do not make the top-level or lib-symbol callers strip `embedded_files` and its closing `)` outside the dedicated parser once the goal is structural parity with KiCad's embedded-files parser flow.
- In `parseLibSymbol()`, keep helper section-head ownership aligned with upstream restored helper boundaries too: `parseBodyStyles()`, `parsePinNames()`, and `parsePinNumbers()` should consume their own section head tokens instead of relying on `parseLibSymbol()` to strip them first.
- In the library draw-item family under `parseLibSymbol()`, keep the current unit/body-style on parser-owned state, like upstream `m_unit` / `m_bodyStyle`, instead of threading `unit_number` / `body_style` through every library draw-item parser call.
- In that same library draw-item family, keep hidden-text user-field ordinal allocation on parser-owned lib-symbol parse state too once the draw-item dispatcher has moved onto parser-owned unit/body-style flow. Do not reintroduce a passed-in symbol reference solely to allocate hidden field ordinals.
...
- In that same shared `parseSchText()` `effects` branch, force visibility back on for the whole shared text family after `parseEDA_TEXT()`, like upstream `text->SetVisible( true )`. Keep the parsed hidden bit in `effects`, but do not leave labels hidden after the effects branch.
...
- In that same loader-side sheet-list flow, preserve sheet identity metadata on each loaded path too. `LoadedSheetPath` should carry the corresponding sheet UUID/name from the hierarchy link instead of collapsing the list to only paths and page numbers, because later post-load stages still need the owning sheet identity.
...
- In that shared textbox cluster, keep `parseSchTextBoxContent()` on caller-owned mutation flow too: `parseSchTextBox()` / `parseSchTableCell()` should construct the owning textbox or cell object first, then hand it into the shared body to mutate, rather than having the shared body allocate and return a detached object.
- Keep schematic table cells on a distinct owning model path too. Do not keep `Table.cells` collapsed to plain `TextBox` values once the goal is structural parity with upstream `SCH_TABLECELL` ownership, even if the shared content parser still mutates both through one textbox-like body.
