...
- `MigrateSimModels`
  ...
  12. migrated `Sim.*` field state now also hydrates a structured `Symbol.sim_model` snapshot during load, so the remaining blocked simulator-model work is no longer forced to live only as flat property strings
