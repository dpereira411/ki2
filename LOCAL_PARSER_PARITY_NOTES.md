...
- `MigrateSimModels`
  ...
  15. the structured snapshot now also derives library-backed state from raw `Sim.Params` payloads when explicit `Sim.Library` / `Sim.Name` fields are absent, so the current model can carry more of the raw-SPICE library/model branch without inventing extra migrated properties
