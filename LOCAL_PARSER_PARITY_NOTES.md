...
- `MigrateSimModels`
  ...
  Model expansion in progress: `SimModel` now preserves ordered parameter payloads, ordered pin mappings, and `Value`-field placeholder binding (`${SIM.PARAMS}` / `${SIM.NAME}`) instead of collapsing that state away immediately, because the remaining resolved-model branch depends on more than deduplicated field maps.
