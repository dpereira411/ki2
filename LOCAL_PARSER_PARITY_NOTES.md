...
- `MigrateSimModels`
  ...
  21. raw-SPICE fallback now also follows upstream `modelFromValueField` behavior in the representable branches: when legacy `Spice_Model` is absent but `Value` supplies the model text, migration now uses that text, rewrites `Value` to `${SIM.PARAMS}`, and locks the resulting structured `sim_model` state across primitive-only and primitive+library cases
  ...
  Model expansion in progress: `SimModel` now preserves ordered parameter payloads and needs the same treatment for pin mappings too, not only deduplicated `BTreeMap`s, because richer project/embedded/control-model branches can depend on source order.
  Remaining blocked gap: the heavier simulator-model / project / embedded-model branch that resolves library-backed models, the still-unported control-source/internal-model families beyond the current `DC/SIN/PULSE/EXP/AM/SFFM/PWL/TRNOISE/TRRANDOM` slice, and full `Spice_*` inference paths. Do not fake that remaining stage without first expanding the Rust model beyond plain parser fields.
