...
- `MigrateSimModels`
  ...
  Model expansion in progress: `SimModel` now preserves ordered parameter payloads, ordered pin mappings, `Value`-field placeholder binding (`${SIM.PARAMS}` / `${SIM.NAME}`), enabled/disabled state, and coarse model origin tagging (raw SPICE vs built-in/internal vs explicit library-reference vs IBIS) instead of collapsing that state away immediately, because the remaining resolved-model branch depends on more than deduplicated field maps.
