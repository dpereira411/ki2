...
- `MigrateSimModels`
  ...
  30. library-backed model-name lookup now also follows KiCad's split-before-resolve flow more closely:
     - modern and migrated `Sim.Name` payloads are stripped at the first whitespace before SPICE/IBIS library lookup instead of trying to resolve the whole inline-params string as the model name
