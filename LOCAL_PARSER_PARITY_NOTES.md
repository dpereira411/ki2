...
- `MigrateSimModels`
  ...
  12. migrated and already-modern `Sim.*` field state now also hydrates a structured `Symbol.sim_model` snapshot during load, including `Sim.Library`, `Sim.Name`, parsed `Sim.Params` name/value maps, and `Sim.Pins` mappings, so the remaining blocked simulator-model work is no longer forced to live only as flat property strings
