...
- `MigrateSimModels`
  ...
  Model expansion in progress: `SimModel` now needs to preserve ordered parameter payloads too, not only a deduplicated `BTreeMap`, because richer project/embedded/control-model branches can depend on source ordering and duplicate keys.
