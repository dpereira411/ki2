...
- `MigrateSimModels`
  ...
  Model expansion in progress: `SimModel` now preserves ordered parameter payloads, ordered pin mappings, `Value`-field placeholder binding (`${SIM.PARAMS}` / `${SIM.NAME}`), enabled/disabled state, and coarse model origin tagging (raw SPICE vs built-in/internal vs explicit library-reference vs IBIS) instead of collapsing that state away immediately, because the remaining resolved-model branch depends on more than deduplicated field maps. The tree also now has explicit sim-library source-stack, content-loading, and filesystem path-resolution helpers for loaded symbols (schematic embedded models, symbol/lib embedded models, project-relative files, then `SPICE_LIB_DIR` fallback), so the remaining blocker is no longer source-location/content plumbing itself.
  ...
  - parsing/resolution of `.kicad_sim` / embedded model content itself on top of the new source-location/content plumbing
