...
#[test]
fn accepts_legacy_class_label_alias_like_upstream() {
    let src = r#"(kicad_sch
  (version 20250114)
  (generator "eeschema")
  (uuid "u-1")
  ...
  (class_label "NETCLASS" (shape rectangle) (at 1 2 0) (uuid "c-1"))
    ...
    let schematic = parse_schematic_file(Path::new(&path)).expect("must accept class_label");
    let label = schematic
        .screen
        .items
        .iter()
        ...
        .expect("class label");
    assert_eq!(label.kind, LabelKind::Directive);
    assert_eq!(label.text, "NETCLASS");
    assert_eq!(label.shape, LabelShape::Rectangle);
    assert_eq!(label.at, [1.0, 2.0]);
    assert_eq!(label.uuid.as_deref(), Some("c-1"));
    let _ = fs::remove_file(path);
}
