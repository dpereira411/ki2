...
#[test]
fn cli_erc_reports_missing_library_configuration_issues() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(
        "../ki/tests/fixtures/erc_upstream_qa/projects/legacy_hierarchy/legacy_hierarchy.kicad_sch",
    );
    let report_path = path.with_file_name("legacy_hierarchy-erc.json");
    ...
    let report = std::fs::read_to_string(&report_path).expect("erc json report");
    let json: serde_json::Value = serde_json::from_str(&report).expect("json report");
    let issues = json["violations"]
        .as_array()
        .expect("violations array")
        .iter()
        .filter(|violation| violation["code"] == "erc-lib-symbol-issues")
        .collect::<Vec<_>>();

    assert!(!issues.is_empty(), "{issues:#?}");
    assert!(issues.iter().all(|violation| {
        violation["severity"] == "Error"
            && violation["message"]
                == "The current configuration does not include the symbol library 'complex_hierarchy'"
    }));
...
#[test]
fn cli_erc_reports_missing_symbol_in_existing_library() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(
        "../ki/tests/fixtures/erc_upstream_qa/projects/issue17771/issue1771.kicad_sch",
    );
    let report_path = path.with_file_name("issue1771-erc.json");
    ...
    assert!(issues.iter().all(|violation| {
        violation["severity"] == "Error"
            && violation["message"]
                == "Symbol 'TST-01025' not found in symbol library 'Mechanical'"
    }));
