...
fn match_live_bus_member_live<'a>(
    ...
    for member in bus_members {
        ...
        if member.kind == ReducedBusMemberKind::Bus {
            if let Some(found) = match_live_bus_member_live(&member.members, search) {
                return Some(found);
            }
        } else if member.local_name == search.local_name {
            return Some(member);
...
fn match_live_bus_member_mut_live<'a>(
    ...
    for member in bus_members {
        ...
        if member.kind == ReducedBusMemberKind::Bus {
            if let Some(found) = match_live_bus_member_mut_live(&mut member.members, search) {
                return Some(found);
            }
        } else if member.local_name == search.local_name {
            return Some(member);
...
// the attached bus connection owner directly. Remaining divergence is that the bus-entry item
// itself is still a reduced carrier rather than a live item/pointer object.
...
// longer carry a copied reduced bus index alongside that live owner.
...
fn refresh_reduced_live_multiple_bus_parent_names_on_handles(
    ...
    for subgraph_handle in live_subgraphs {
        let subgraph_snapshot = subgraph_handle.borrow().clone();
